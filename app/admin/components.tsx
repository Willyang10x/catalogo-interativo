...
export function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Dispara a função segura lá no Servidor
    const res = await authenticate(password);
    
    if (res.success) {
      router.refresh(); // Manda o Next.js recarregar a página pelo servidor para ler o cookie
    } else {
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            ...
            <input 
              ...
              className="w-full px-5 py-4 bg-zinc-950/50 border border-zinc-800 rounded-xl text-white outline-none focus:border-[#00ff66] transition-colors font-mono disabled:opacity-50"
            />
          ...
          <button 
            ...
            className="w-full py-4 bg-white text-black hover:bg-[#00ff66] hover:shadow-[0_0_20px_rgba(0,255,102,0.3)] font-black uppercase tracking-wider text-sm rounded-xl transition-all duration-300 disabled:opacity-50"
          >
...
export function Dashboard() {
  ...
  useEffect(() => setInventory(products), []);
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-[#00ff66] selection:text-black">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-4">
              ...
              <span className="hidden md:inline-block px-2 py-1 bg-green-900/50 border border-green-500/50 rounded text-xs font-bold text-green-400 uppercase tracking-widest">Seguro</span>
            </div>
            <div className="flex items-center gap-4">
              <Link href="/" className="text-sm font-semibold text-zinc-400 hover:text-white transition-colors">Ver Loja</Link>
              <button onClick={handleLogout} className="text-sm font-bold bg-zinc-800 hover:bg-red-500/20 hover:text-red-500 text-zinc-300 px-4 py-2 rounded-lg transition-colors border border-transparent hover:border-red-500/50">
                Sair
              </button>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in-up">
        <div className="mb-8">
          ...
          <p className="text-zinc-400">Dados protegidos por autenticação de servidor.</p>
        </div>
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {inventory.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td className="p-4 text-sm font-bold text-[#00ff66]">{item.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                    <td className="p-4 text-right">
                      <button className="text-zinc-500 hover:text-white transition-colors text-sm font-semibold underline decoration-zinc-700 underline-offset-4">Editar</button>
                    </td>
                  </tr>
                ))}
              </tbody>
